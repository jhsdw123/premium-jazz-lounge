...
export async function generateTitleCandidates({
  ...
  const promptParts = [
    'You are naming a short instrumental jazz piece.',
    ...
    '',
    'CONSTRAINT: Avoid sharing 2 or more meaningful words with ANY existing title below.',
    avoidSampled.length
    ...
    '',
    `Generate ${count} candidate titles, all distinct from each other.`,
    attempt > 1
      ? '(Previous attempt collided with existing titles. Be more creative — different vocabulary, fresh imagery.)'
      : '',
  ].filter(Boolean);
